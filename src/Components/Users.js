...
const Users = () => {
  ...
  const getUsers = () => {
    // retrive the users
    axios
      .get(process.env.REACT_APP_ENDPOINT + "/api/users")
      .then((response) => {
  ...
  const addUser = (data) => {
    ...
    axios
      .post(process.env.REACT_APP_ENDPOINT + "/api/createuser", payload)
      .then(() => {
        getUsers();
        setSubmitted(false);
        setIsEdit(false);
  ...
  const updateUser = (data) => {
    ...
    axios
      .put(process.env.REACT_APP_ENDPOINT + "/api/updateuser", payload)
      .then(() => {
        getUsers();
        setSubmitted(false);
        setIsEdit(false);
  ...
  const deleteUser = (data) => {
    // console.log(data);
    axios
      .post(process.env.REACT_APP_ENDPOINT + "/api/deleteuser", data)
      .then(() => {
        getUsers();
      })
      .catch((error) => {
